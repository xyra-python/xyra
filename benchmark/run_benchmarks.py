...
def kill_process_on_port(port):
    """Safely kill any process listening on the target port."""
    try:
...
def run_benchmark(name, start_cmd, port, path="/"):
    ...
    try:
        ...
        subprocess.run(["pkill", "-f", "python.*app_"], check=False)
        subprocess.run(["pkill", "-f", "uvicorn"], check=False)
        subprocess.run(["pkill", "-f", "gunicorn"], check=False)
        subprocess.run(["pkill", "-f", "app_go_native"], check=False)
        subprocess.run(["pkill", "-f", "app_go_gin"], check=False)
        kill_process_on_port(port)
        time.sleep(2)

if __name__ == "__main__":
    print("Building Go applications...")
    subprocess.run(["go", "build", "-o", "app_go_native", "app_go_native.go"], check=True)
    subprocess.run(["go", "build", "-o", "app_go_gin", "app_go_gin.go"], check=True)
