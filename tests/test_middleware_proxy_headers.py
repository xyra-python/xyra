...
def test_proxy_headers_chain_recursive_trust():
    ...
    request, response = create_request("10.0.0.1", {"X-Forwarded-For": "1.1.1.1, 10.0.0.2"})
...
def test_proxy_headers_invalid_xff():
    request, response = create_request("10.0.0.1", {"X-Forwarded-For": "invalid, 1.2.3.4"})
    mw = ProxyHeadersMiddleware(["10.0.0.1"])
    mw(request, response)
...
def test_proxy_headers_mixed_ipv4_ipv6():
    ...
    request, response = create_request("::1", {"X-Forwarded-For": "1.2.3.4, 2001:db8::2"})
