...
def test_ratelimiter_concurrent_access():
    ...
    def make_request_wrapper(i):
        """Wrapper to capture the loop variable properly."""
        def inner():
            allowed = limiter.is_allowed(client_key)
            remaining = limiter.get_remaining_requests(client_key)
            results.append((i, allowed, remaining))
        return inner

    for i in range(10):
        thread = threading.Thread(target=make_request_wrapper(i))
        threads.append(thread)
...
def test_ratelimiter_cleanup_thread_safety():
    ...
    # Manually add old timestamps (without lock since it doesn't exist)
    limiter._requests[client_key] = [current_time - 10, current_time - 5]  # Old timestamps
