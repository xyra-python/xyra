...
@pytest.mark.asyncio
async def test_path_traversal_url_encoded_blocked():
    """Test that URL-encoded path traversal attempts are blocked in static file serving."""
    # Setup directories
    ...
    try:
        ...
        mock_native_req.get_url.return_value = "/static/%2e%2e%2fsecret.txt"
        mock_native_req.get_parameter.return_value = "%2e%2e%2fsecret.txt"
...
@pytest.mark.asyncio
async def test_static_files_url_decoding():
    """Test that static file server properly URL-decodes the path for valid files."""
    temp_dir = tempfile.mkdtemp()
    try:
        static_dir = os.path.join(temp_dir, "static")
        os.makedirs(static_dir)

        # Create safe file with a space in the name
        with open(os.path.join(static_dir, "my space file.txt"), "w") as f:
            f.write("SPACE_FILE")
        ...
        # Mock Native Request with url-encoded space
        mock_native_req = Mock()
        mock_native_req.get_method.return_value = "GET"
        mock_native_req.get_url.return_value = "/static/my%20space%20file.txt"
        mock_native_req.get_parameter.return_value = "my%20space%20file.txt"
        mock_native_req.for_each_header = Mock(side_effect=lambda func: None)
        ...
        # Check what was sent
        args = mock_native_res.end.call_args
        assert args is not None, "File content was not sent!"
        sent_data = args[0][0]
        if isinstance(sent_data, str):
            sent_data = sent_data.encode()

        assert b"SPACE_FILE" in sent_data, "URL decoding failed, file not served."
    finally:
        shutil.rmtree(temp_dir)
